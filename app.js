...
После настройки бот начнёт работать полностью автономно - вам останется только наблюдать за результатом 🚀
  ...
  },
  {
    reply_markup: {
      inline_keyboard: [
        [{ text: "Частые вопросы?", callback_data: "often_questions" }],
        [{ text: "<< Назад", callback_data: "menu_back" }]
      ]
    },
  });
...
bot.action("often_questions", async (ctx) => {
  ctx.editMessageMedia({
    type: "photo",
    media:"https://i.ibb.co/LhRgJzLX/card-how-it-works-prime-Wave.jpg", 
    caption: `<b>Частые вопросы?</b>

<blockquote><b>• Насколько это безопасно для аккаунта?</b>
Мы используем только официальные инструменты Telegram, поэтому риски минимальны.

<b>• Почему я должен доверять вам?</b>
Сервисом уже пользуются десятки клиентов для работы с комментариями и набором аудитории.

<b>• Откуда такая низкая стоимость - это СКАМ?</b>
Нет, это не СКАМ. Стоимость формируется так: часть идёт на оплату серверов, а остальная — на поддержку и развитие проекта.

<b>• Что будет, если подписка закончится?</b>
Работа бота будет приостановлена до продления подписки. Все настройки сохраняются.

<b>• Что делать, если бот не отправил комментарий?</b>
Проверьте доступ к комментариям и статус подписки. В случае ошибки бот отправит уведомление.

<b>• Поддерживаются ли приватные каналы?</b>
На данный момент работа с приватными каналами не поддерживается. Возможность может быть добавлена в будущем.
  ...
  },
  {
    reply_markup: {
      inline_keyboard: [
        [{ text: "<< Назад", callback_data: "how_it_works" }]
      ]
    },
  });
});
...
<blockquote><b>Я мониторю выбранные каналы и автоматически оставляю комментарии в числе первых - быстро и точно.</b></blockquote>
`,
    parse_mode: "HTML",
    reply_markup: {
      inline_keyboard: [
